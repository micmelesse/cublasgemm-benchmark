...
void CPU_fill_rand_init(_Float16 *A, int nr_rows_A, int nr_cols_A) {
        ...
        for(int i = 0; i < nr_rows_A * nr_cols_A; i++){
                ...
                A[i] = r.Value;
                if(count<=5){
			printf("Matrix init:Frac0 %d, Exp %d, Value:%d final value %d, \
				random value:%d\n", r.BitArea.Frac0, r.BitArea.Exp, r.Value,\
										 A[i], rand());
                        count++;
                }
        }
...
void CPU_fill_rocblas_rand_narrow_init(_Float16 *A, int nr_rows_A, int nr_cols_A) {
...
void CPU_fill_sin(_Float16 *A, int nr_rows_A, int nr_cols_A) {
...
// Fill the array A(nr_rows_A, nr_cols_A) with trig random numbers on CPU
void CPU_fill_cos(_Float16 *A, int nr_rows_A, int nr_cols_A) {
...
void CPU_fill_all_zero(_Float16 *A, int nr_rows_A, int nr_cols_A) {
...
void CPU_fill_narrow_rand(_Float16 *A, int nr_rows_A, int nr_cols_A) {
	...
	union Float16Rand
		...
		if (r.BitArea.Exp == 0x80){
                ...
                A[i] = r.Value;
                if(count<=5){
                        printf("Matrix init:Frac %d, Exp %d, Value:%f final value %f,\
				 random value:%d\n", r.BitArea.Frac0, r.BitArea.Exp, \
								r.Value, A[i], rand());
                        count++;
                }
        }
...
void CPU_fill_const_init(_Float16 *A, int nr_rows_A, int nr_cols_A) {
	_Float16 x = 0.333984375;
	for(int i = 0; i < nr_rows_A * nr_cols_A; i++){
		A[i] = x;
	}
}
...
int main(int argc, char ** argv)
{
	...
	_Float16 *h_A = (_Float16 *)malloc(max_m_k_n * k1 * sizeof(_Float16));
	_Float16 *h_B = (_Float16 *)malloc(max_m_k_n * k1 * sizeof(_Float16));
	_Float16 *h_C = (_Float16 *)malloc(max_m_k_n * max_m_k_n * sizeof(_Float16));
	...
	checkCuda(hipMalloc(&d_A, max_m_k_n * k1 * sizeof(_Float16)));
	checkCuda(hipMalloc(&d_B, max_m_k_n * k1 * sizeof(_Float16)));
	checkCuda(hipMalloc(&d_C, max_m_k_n * max_m_k_n * sizeof(_Float16)));

	checkCuda(hipMemcpy(d_A,h_A,max_m_k_n * k1 * sizeof(_Float16),hipMemcpyHostToDevice));
	checkCuda(hipMemcpy(d_B,h_B,max_m_k_n * k1 * sizeof(_Float16),hipMemcpyHostToDevice));
	checkCuda(hipMemcpy(d_C,h_C,max_m_k_n * max_m_k_n * sizeof(_Float16),hipMemcpyHostToDevice));
	...
	for(vector<vector<int>>::iterator idx = sizes.begin(); idx !=sizes.end(); idx++){
		...
		cout << " matrix (16): " \
			...
			ldb << " -ldc: " << ldc << ", ops: " << " average time: " << time << " sec "\
