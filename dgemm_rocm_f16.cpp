...
void CPU_fill_rand_init(_Float16 *A, int nr_rows_A, int nr_cols_A) {
        ...
        for(int i = 0; i < nr_rows_A * nr_cols_A; i++){
                ...
                r.BitArea.Frac0 = rand() % 0x400;
                r.BitArea.Exp = rand() % 0x1f;
                r.BitArea.Signed = rand() & 0x1;
                //If mant is out of scope, keep looping

                A[i] = r.Value;
                if(count<=5){
			printf("Matrix init values:%f\n", A[i]);
                        count++;
                }
        }
}
...
void CPU_fill_narrow_rand(_Float16 *A, int nr_rows_A, int nr_cols_A) {
	...
	union Float16Rand
        ...
        for(int i = 0; i < nr_rows_A * nr_cols_A; i++){
                ...
                r.BitArea.Frac0 = rand() % 0x400;
                r.BitArea.Exp = rand() % 0x11;
		if (r.BitArea.Exp == 0x10){
                        r.BitArea.Frac0 &= 0x1ff;
                }
                r.BitArea.Signed = rand() & 0x1;
                //If mant is out of scope, keep looping
...
void CPU_fill_const_init(_Float16 *A, int nr_rows_A, int nr_cols_A) {
	_Float16 x = 0.333251953125;
	int count=1;
	for(int i = 0; i < nr_rows_A * nr_cols_A; i++){
		A[i] = x;
                if(count<=1){
			printf("binary rep of constant %x\n", A[i]);
                        count++;
                }
	}
}
