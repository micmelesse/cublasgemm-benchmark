...
void CPU_fill_rand_init(double *A, int nr_rows_A, int nr_cols_A) {
        ...
        for(int i = 0; i < nr_rows_A * nr_cols_A; i++){
                ...
                A[i] = r.Value;
                if(count<=5){
printf("Matrix init:Frac0 %d, frac1: %d, Exp %d, Value:%e final value %e, random value:%d\n", r.BitArea.Frac0,r.BitArea.Frac1, r.BitArea.Exp, r.Value, A[i], rand());
                        count++;
                }
        }
}
...
void CPU_fill_narrow_rand(double *A, int nr_rows_A, int nr_cols_A) {
        ...
        for(int i = 0; i < nr_rows_A * nr_cols_A; i++){
                ...
                A[i] = r.Value;
                if(count<=5){
                        printf("Matrix init:Frac %d, %d, Exp %d, Value:%e final value %e, random value:%d\n", r.BitArea.Frac0, r.BitArea.Frac1, r.BitArea.Exp, r.Value, A[i], rand());
                        count++;
                }
        }
}
...
int main(int argc, char ** argv){
  ...
  switch(hash_func(init_command)) {
    ...
    const double *beta = &bet;
  	...
  	__half *d_A, *d_B, *d_C;
    checkCuda(hipMalloc(&d_A, max_m_k_n * max_m_k_n * sizeof(__half)));
    checkCuda(hipMalloc(&d_B, max_m_k_n * max_m_k_n * sizeof(__half)));
    checkCuda(hipMalloc(&d_C, max_m_k_n * max_m_k_n * sizeof(__half)));
...
//          ldc = m;
 ...
 // printf("Running venkat3...\n");
  stat = hipblasDgemm(handle, HIPBLAS_OP_N, HIPBLAS_OP_T, m, n, k, alpha, d_A, lda, d_B, ldb, beta, d_C, ldc); 
 // printf("Running venkat4...\n");
#else
	stat = hipblasHgemm(handle, HIPBLAS_OP_N, HIPBLAS_OP_T, m, n, k, alpha, d_A, lda, d_B, ldb, beta, d_C, ldc); 
#endif
